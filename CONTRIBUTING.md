...
Thank you for considering contributing to **Jekwwer/pgsql-workspace**!
...
2. **Clone Your Fork**:
   ...
   ```bash
   git clone https://github.com/<YOUR_USERNAME>/pgsql-workspace.git
   cd pgsql-workspace
   ```
...
3. **Set Up Upstream Remote**:
   ...
   ```bash
   git remote add upstream https://github.com/Jekwwer/pgsql-workspace.git
   ```
...
- **Dependabot:**
  The `.github/dependabot.yml` file monitors and updates dependencies for npm packages, pip packages and GitHub Actions.
...
- **Database Service:**
  Managed via `docker-compose.yml` for PostgreSQL.
...
- **sqlfluff:check:** Lints SQL files.
- **sqlfluff:write:** Fixes all fixable SQL lint errors.
- **prettier:check:** Verifies code formatting for non-SQL files.
- **prettier:write:** Auto-formats code for non-SQL files.
...
Your contributions make **Jekwwer/pgsql-workspace** better.
